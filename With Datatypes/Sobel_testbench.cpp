...
CCS_MAIN(int argc,char *argv[]){
	...
	int image[N][M];
	int gx[N][M],gy[N][M],out_image[N][M],out_image_angle[N][M];
	...
	//Running Algorithm
	run_algorithm((pixel(*)[352])image,(root(*)[352])gx,(root(*)[352])gy,(root(*)[352])out_image,(root(*)[352])out_image_angle);
	
	//Reusing image to read the check values USE AFTER CREATING THE FILES FIRST
	read(image,checkdir);
